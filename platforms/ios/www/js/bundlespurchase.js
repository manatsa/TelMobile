...
function selectCrossNetBundles() {
    ...
    var Values = ["1.00", "2.00", "5.00", "10.00"];
    var text = "<option disabled selected>Select a voice bundle</option>";
    for (var i = 0; i < Options.length; i++) {
        text += "<option value='" + Values[i] + "' >" + Options[i] + "</option>"
    }
...
function selectDataBundles() {
    ...
    var Values = ["0.50", "1.00", "1.00", "2.00", "3.00", "5.00", "10.00", "20.00", "45.00", "75.00"];
...
function selectSMSBundles() {
    ...
    var values = ["0.25", "0.50", "1.50", "3.00"];
...
function getBundleDescription(selected) {
    switch (selected) {
        case "I":
            desc = "Telecel offers on international voice bundles that allow you call any international number affordably.  All you need to do is juice up and dial *---# to buy a bundle of you choice. ";
            break;
        case "A":
...
$("#divProductBundlePurchaseTabs").tabs({collapsible: true, active: false});
